...
The machine token is `{token}`. In prose that reads
...
{len(cov['emitted'])} of {len(cov['declared'])} declared gates emitted and all
...
**`R1L_STAGE2R_PHYSICAL_SOURCE_MATERIALITY_NOT_MET`** — at SNR₀ = {SNR_P:.0f} on
...
SNR₀ = {SNR_S:.0f}, on the same non-negative banks, the resolved arm meets every
