...
E3D = "artifacts/reports/E3D_SOURCE_CLASS_STRESS.md"
SCORE = "src/phrt/metrics/scoring.py"
MORPH = "src/phrt/metrics/morphology.py"
...
def build(c: str) -> list[dict]:
    ...
    r2_pins = {
        ...
        "quadrature": R("legacy screen/ray quadrature of the spectral "
                        "calculation; the source-norm integration used for the "
                        "localization readback is a SEPARATE quadrature and is "
                        "pinned on C04", R2MD),
    ...
    r1_pins = {
        ...
        "covariance": R("whitened diagonal at SNR_0 = 100; eight noisy draws "
                        "per truth plus one excluded noiseless control; "
                        "independent unit is the truth", R1FZ),
        ...
        "evaluation_metric": R("baseline-inclusive field loss "
                               "||W_a(D c_hat - v)|| / max(||W_a v||, eta) on "
                               "the fixed evaluation grid, normalised Gaussian "
                               "window h = 3 M; anchored span at epsilon 0.25, "
                               "q 0.95, floor 8 M. NOT a coefficient-space "
                               "metric", SCORE),
    }
    hmt2_pins = {
        "operator": R("order-labelled arms with UNRESOLVED_IMAGE and "
                      "TOTAL_FLUX controls", HMT2),
        "source_basis": R("L896_radial_enriched 8x7x16 nominal 896, 768 "
                          "fitted contrast coefficients after m=0 removal; "
                          "L448_contrast 4x7x16 nominal 448, 384 fitted",
                          HMT2),
        "source_Gram": NA("the morphology loss is a state-dependent error on "
                          "reconstructed maps, not a source-norm quadratic "
                          "form; no Gram matrix enters the endpoint"),
        "sampled_rays": R("frozen HMT-2 ray set", HMT2),
        "quadrature": R("pixel-integrated, corrected row convention", HMT2),
        "covariance": R("a common standard-normal order/ray/time tensor per "
                        "truth/draw, mapped through each arm's "
                        "noise_from_standard readout; the TOTAL_FLUX control "
                        "uses an all-ones order mixer plus a total-flux "
                        "collapse. Four paired draws per truth is REPLICATION, "
                        "not the covariance", HMT2),
        ...
        "evaluation_metric": R("state-dependent morphology error: assignment "
                               "cost for SINGLE_RESOLVED/MULTI_RESOLVED, "
                               "descriptor discrepancies for BLENDED/"
                               "AMBIGUOUS, amplitude for DEAD; all_state_error "
                               "is their mean. Materiality median >= 0.10 and "
                               "bootstrap lower bound >= 0.05", MORPH),
    }
    d026_pins = {
    ...
    return [
        ...
        cl("C04",
           ...
           {**r2_pins,
            "quadrature": R("Gauss radial plus exact piecewise-quadratic "
                            "temporal integration of the saved coefficient "
                            "vectors; a source-norm integration, not the "
                            "spectral calculation's screen quadrature", LOC),
            "source_Gram": R("source Gram retained with its cross terms; "
                             "relative refinement steps 1.83e-14 and 4.33e-15 "
                             "across 64/128/256 radial refinements", LOC),
            "evaluation_metric": R("interval source-energy fraction of the "
                                   "squared source norm; NOT a Fisher trace",
                                   LOC),
            "covariance": NA("a localization of saved coefficient vectors; no "
                             "observation noise enters"),
            "noise_normalisation": NA("as above"),
            "sampled_rays": NA("postprocessing of two saved vectors; no ray "
                               "set is read")},
           None, True,
        ...
        cl("C05",
           ...
           r2_pins, None, True,
           "the scoped legacy-measure fact does not wait on an unperformed "
           "corrected-measure run; the corrected-measure count is a separate "
           "claim, C05b"),
        cl("C05b",
           "036/037: the corrected-measure operational count",
           "section 6.2: not stated; only the one-to-two reweighting interval "
           "is given, under its fixed row-noise and metric assumptions",
           "DEFERRED_PENDING_A_CORRECTED_MEASURE_RUN", D,
           [ev(c, R2MD, "operational counts and the reweighting discussion")],
           {**r2_pins,
            "evaluation_metric": U("the corrected-measure operational count "
        ...
        cl("C13",
           ...
           "hull error",
           "SCOPE_NARROWED_AND_METRIC_CORRECTED", D,
           [ev(c, G1C, "band-normalised tessellation discrepancy between the "
                       "named refinement levels")],
           {"operator": R("curved band hull boundary at the qualified "
                          "candidate", G1C),
            "source_basis": NA("a hull boundary, not a source representation"),
            "source_Gram": NA("as above"),
            "sampled_rays": NA("a boundary geometry, not a ray set"),
            "quadrature": R("polygon tessellation of the fitted boundary, "
                            "compared between refinement levels", G1C),
            "covariance": NA("a geometric qualification; no noise model"),
            "noise_normalisation": NA("as above"),
            "target_nuisance_partition": NA("no target/nuisance split"),
            "estimator": NA("no estimator"),
            "evaluation_metric": R("band-normalised geometric discrepancy "
                                   "9.592e-08 against the 1e-6 numerical "
                                   "integration component budget; NOT the "
                                   "detector-response norm", G1C)},
           None, True),
        cl("C14",
           ...
           {"operator": R("path-domain predicate: the requested crossing "
                          "located in Mino parameter against the source "
                          "annulus and escape endpoints", N033),
            "source_basis": NA("a per-ray domain predicate; no source "
                               "representation enters"),
            "source_Gram": NA("as above"),
            "sampled_rays": R("66 development point/order IDs and 192 "
                              "previously unused confirmation IDs, frozen "
                              "before the phase ran", N033),
            "quadrature": R("adaptive radial quadrature with the index-excluded "
                            "reduced product, epsabs and epsrel 1e-12; graded "
                            "panel reference; margin factor 10", N033),
            "covariance": NA("a domain predicate, not a measurement; no "
                             "observation noise model applies"),
            "noise_normalisation": NA("as above"),
            "target_nuisance_partition": NA("no target/nuisance split"),
            "estimator": NA("adjudication, not estimation"),
            "evaluation_metric": R("label agreement between the primary and "
                                   "two references, zero forced labels; shared "
                                   "inputs are the roots, the angular crossing "
                                   "and the path classification", N033)},
           None, False),
        cl("C17",
           "037 section 6.4: 'enriching the declared temporal class from 224 "
           "to 1056 dimensions'",
           "section 6.4: the four-class ladder C224, C448_T, C528_S, "
           "C1056_ST, changing both spatial and temporal factors; median "
           "operational-rank fraction 0.897 to 0.729 while the rank itself "
           "rises 201 to 770; sigma_min+ 1.635e-02 to 1.332e-07",
           "CORRECTED_LADDER_AND_FIELDS", D,
           [ev(c, E3D, "class ladder table (radial/azimuthal/temporal/"
                       "dimension) and the richness table columns "
                       "'operational rank (median)', 'as a fraction', "
                       "'sigma_min+'")],
           {**e3c_pins,
            "source_basis": R("C224 4x7x8, C448_T 4x7x16, C528_S 6x11x8, "
                              "C1056_ST 6x11x16", E3D),
            "evaluation_metric": R("median operational rank and its fraction "
                                   "of class dimension, and sigma_min+, at the "
                                   "operational threshold at SNR_0 = 100", E3D)},
           None, True),
        cl("C18",
           "037 section 5.2 / 1.2: the localized-directional depth quoted "
           "beside the E3C scalar probe",
           "section 6.4: the deepest retarded age whose best-determined "
           "localized mode clears the operational threshold moves by at most "
           "one grid step of 4 M among physical and mechanism arms, and every "
           "move appears at a spatial enrichment step and none at the temporal "
           "one; this is not the E3C scalar detectability probe",
           "SEPARATED_FROM_THE_E3C_PROBE", D,
           [ev(c, E3D, "'Recoverable depth against class richness' table and "
                       "the asymmetry paragraph following it")],
           {**e3c_pins,
            "evaluation_metric": R("deepest retarded age whose best-determined "
                                   "localized mode clears the operational "
                                   "threshold at SNR_0 = 100", E3D)},
           None, False),
        cl("C19",
           "037 section 1.2: 'a pairing-permuted control is better conditioned "
           "than the physical operator at 12 of 12 geometries across 16 frozen "
           "seeds'",
           "section 1.2, unchanged, with the seed count and the worst-seed "
           "statement sourced",
           "RETAINED_WITH_EVIDENCE_ADDED", D,
           [ev(c, E3C, "'Negative control: PAIRING_DESTROYED over 16 frozen "
                       "seeds' section, including the worst-of-sixteen "
                       "statement")],
           {**e3c_pins,
            "operator": R("PAIRING_DESTROYED: each order's delays, positions "
                          "and weights permuted independently, preserving all "
                          "three marginals", E3C),
            "evaluation_metric": R("conditioning of the whitened operator, "
                                   "compared against the physical resolved "
                                   "operator at each geometry", E3C)},
           None, True),
        cl("C20",
           "037 section 7.4: FAMILY_HETEROGENEITY supported by an L448 vs L896 "
           "row",
           "section 7.4: 5 of 12 family-estimator cells material on the "
           "physical target and 4 of 12 on both; circular_hotspot_trajectory "
           "negative under both estimators; class dependence recorded "
           "separately as CLASS_DEPENDENCE",
           "CORRECTED_A_CLASS_RESULT_WAS_LABELLED_A_FAMILY_RESULT", D,
           [ev(c, EVID, "HMT-2 per-family table and the "
                        "HMT2_MAIN_FAMILY_HETEROGENEITY_PRESERVED entry")],
           hmt2_pins, None, True),
        cl("C21",
           "no equivalent in 036 or 037",
           "Appendix A.1: the R1 field loss, its window, its quadratic-form "
           "computation, the endpoint rule and the bootstrap",
           "NEW_METHOD_DEFINITION", D,
           [ev(c, SCORE, "AgeScorer.build, truth_terms, errors, "
                         "evaluation_grid"),
            ev(c, R1FZ, "/primary, /noise, /bootstrap, /metrics, "
                        "/sealed_bank/n_per_regime, /estimators")],
           {**r1_pins,
            "evaluation_metric": R("E_jb(a) = ||W_a(D c_hat - v)|| / "
                                   "max(||W_a v||, eta), h = 3 M, on the fixed "
                                   "evaluation grid with equal spatial weights",
                                   SCORE)},
           "the executed eta scalar and the frozen evaluation-grid shape are "
           "execution inputs not yet resolved to their recorded payload",
           False),
        cl("C22",
           "no equivalent in 036 or 037",
           "Appendix A.2: the state-dependent HMT-2 morphology metric",
           "NEW_METHOD_DEFINITION", D,
           [ev(c, MORPH, "state_error, assignment_error, blended_error, "
                         "dead_error, aggregate_all_states")],
           hmt2_pins,
           "the order of aggregation over ages and draws in the final paired "
           "reduction lives in the endpoint runner's imported paired_relative, "
           "which this revision has not traced",
           False),
        cl("C15",
...
def main() -> int:
    ...
    (OUT / "CLAIM_EVIDENCE_MATRIX_038.json").write_text(
