...
DRAFT = "Photon_Ring_Retarded_Time_Tomography_Mahakal_Working_Draft_037.md"
...
LEDGER = """# MANUSCRIPT_CORRECTION_LEDGER_037
...
GAPS = """# SUBMISSION_GAPS_037

## A. Editorial — no new science

| item | state |
| --- | --- |
| Regressions R01-R12 repaired and scanned | done, `REGRESSION_SCAN_037.json` |
...
def main() -> int:
    ...
    sc = scan(text)
    (OUT / "REGRESSION_SCAN_037.json").write_text(json.dumps({
        "ruling": "PAPER_I_MANUSCRIPT_REVIEW_037",
        "draft": DRAFT, "draft_sha256": sha(draft), **sc}, indent=2) + "\n")
    (OUT / "MANUSCRIPT_CORRECTION_LEDGER_037.md").write_text(LEDGER)
    (OUT / "SUBMISSION_GAPS_037.md").write_text(GAPS)

    check = json.loads((OUT / "CLAIM_MATRIX_CHECK_037.json").read_text())
    matrix = json.loads((OUT / "CLAIM_EVIDENCE_MATRIX_037.json").read_text())
    blocked = (not sc["all_clean"]) or check["status"] != \
        "SCHEMA_READY_FOR_SEMANTIC_REVIEW"
    status = ("MANUSCRIPT_037_BLOCKED" if blocked
              else "MANUSCRIPT_037_READY_FOR_SEMANTIC_REVIEW")
    completion = {
        "schema": "phrt-completion/1",
        "id": "MANUSCRIPT_037_COMPLETION",
        "ruling": "PAPER_I_MANUSCRIPT_REVIEW_037",
    ...
    (OUT / "MANUSCRIPT_037_COMPLETION.json").write_text(
