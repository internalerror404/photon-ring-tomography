#!/usr/bin/env python3
"""M0 of ruling 032: the interpretation overlay. Zero physical queries.

Nothing under 031 is rewritten. The blocked token, the counters, the printed
figures and every byte of the I0/I1/I2 run directories stay exactly as they
were written; this file records what those numbers do and do not mean.
"""
...
TEXT = """# INTERPRETATION_OVERLAY_032
...
def main(out: Path) -> int:
    ...
    (out / "INTERPRETATION_OVERLAY_032.md").write_text(TEXT)
    rec = {
        "stage": "M0", "ruling": "PAPER_I_MATCHED_INTEGRATION_RULING_032",
        "generated_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "commit": subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT,
                                 capture_output=True, text=True).stdout.strip(),
        "physical_queries": 0, "files_modified_under_previous_rulings": [],
        "old_blocked_token_preserved": "DOMAIN_INTEGRATION_031_BLOCKED",
    }
    (out / "OVERLAY_PROVENANCE_032.json").write_text(
        json.dumps(rec, indent=2) + "\n")
    print(json.dumps({"stage": "M0", "out": str(out.relative_to(ROOT))}))
    return 0
