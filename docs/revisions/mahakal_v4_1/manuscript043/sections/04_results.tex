...
\subsection{Order attribution of the two surviving combinations}\label{sec:orderattr}
Order attribution shows that this result is not driven by the second indirect
image. Recomputing the nuisance projection after omitting order $n=2$, the
$n=0,1$ stack retains two operational singular values, $1.4098$ and $1.2954$,
and $87.2\%$ of the full conditional Fisher trace. Its supported
two-dimensional subspace differs from the full-stack subspace by at most
$3.21^\circ$. In the full-stack modes, $96.90\%$ and $95.64\%$ of the
residualized response energy lie in order $n=1$; omitting $n=1$ removes all
above-threshold historical directions. The direct order has zero raw target
response but improves conditional information by constraining nuisance
coefficients. These are order-dependence results inside the frozen finite
operator, not validation of the continuous acquisition.

\begin{table}[htbp]
\centering\small
\caption{Nuisance-adjusted spectrum by retained image order, at the reference
geometry under the frozen legacy measure. Each subset's nuisance projector is
recomputed from its own rows; the noise level, the 72/152 split and the source
Gram are held fixed. Omitting $n=2$ preserves the operational count; omitting
$n=1$ destroys it. \Cref{app:orderattr} gives all seven subsets.}
\label{tab:orderattr}
\begin{tabular}{l r r r c r}
\toprule
Orders retained&$s_1$&$s_2$&$s_3$&Operational count&$\tr F_{\rm cond}$\\
\midrule
$1$&1.3947&1.2706&0.7491&2&6.0738\\
$0,1$&1.4098&1.2954&0.7606&2&6.2768\\
$0,2$&0.2356&0.2262&0.2103&0&0.4546\\
$0,1,2$&1.4372&1.3459&0.8063&2&7.1983\\
\bottomrule
\end{tabular}
\end{table}

The direct order's contribution deserves its own statement, because it is not
a response effect. Its target columns are identically zero, yet adding it to
$\{1\}$ raises the conditional trace from $6.0738$ to $6.2768$. A nuisance fit
chosen to mimic the order-1 target response must also remain compatible with
the direct observations, so a channel can improve identifiability of a target
it does not observe by constraining the alternative explanations of the
signal. Measuring target energy per order alone would score this contribution
at zero.
